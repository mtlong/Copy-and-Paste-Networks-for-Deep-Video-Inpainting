...
def get_arguments():
    ...
    parser.add_argument("-R", type=str, default="./test/")
...
Pset = DAVIS_Test(root=DATA_ROOT, imset='example_list.txt', size='half')
...
for i, V in tqdm.tqdm(enumerate(Trainloader)):
    ...
    for t in range(2): # forward : 0, backward : 1
        ...
        for f in index:
            ...
            save_path = os.path.join(RESULT_FOLDER, seq_name)
            if t == 1:
    ...
    vid_path = os.path.join(RESULT_FOLDER, '{}.mp4'.format(seq_name))
    frame_path = os.path.join(RESULT_FOLDER, seq_name, 'f%d.jpg')
